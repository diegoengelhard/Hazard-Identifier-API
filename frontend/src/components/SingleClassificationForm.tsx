...
function SingleClassificationForm({ products, onSubmit, isLoading }: Props) {
  ...
  const [selectedProducts, setSelectedProducts] = useState<MultiValue<ProductOption>>([]);
  ...
  const removeProduct = (productToRemove: ProductOption) => {
    setSelectedProducts(selectedProducts.filter(p => p.value !== productToRemove.value));
  };
  
  return (
    <div className="bg-white p-6 rounded-lg shadow-md h-full flex flex-col">
      <h2 className="text-xl font-bold text-gray-800 mb-6">Single Classification</h2>
      <form onSubmit={handleSubmit} className="flex flex-col flex-grow space-y-4">
        <div>
          <label htmlFor="bookingId" className="block text-xs font-semibold text-gray-500 tracking-wide uppercase">Booking ID</label>
          <input
            id="bookingId"
            type="text"
        ...
        <div>
          <label htmlFor="description" className="block text-xs font-semibold text-gray-500 tracking-wide uppercase">Description</label>
          <textarea
            id="description"
            rows={4}
        ...
        <div>
          <label htmlFor="internalNotes" className="block text-xs font-semibold text-gray-500 tracking-wide uppercase">Internal Notes</label>
          <textarea
            id="internalNotes"
            rows={3}
            className="mt-1 block w-full rounded-md border border-gray-300 bg-white/60 px-3 py-2 text-sm placeholder:text-gray-400 focus:border-gray-400 focus:ring-0 resize-y"
        ...
        <div>
          <label htmlFor="products" className="block text-xs font-semibold text-gray-500 tracking-wide uppercase">Products</label>
          <Select
            inputId="products"
            isMulti
            name="products"
        ...
        {selectedProducts.length > 0 && (
          <div className="flex flex-wrap gap-2 pt-2 border-t border-gray-200">
            {selectedProducts.map(product => (
              <div key={product.value} className="bg-gray-200 text-gray-700 text-xs font-medium px-3 py-1 rounded-full flex items-center">
